...
class PrescriptionParser(MedicalDocParser):
    ...
    def get_field(self, field_name):
        ...
        def medicines_directions_extractor(text):
            ...
            # Strip any string which has length less than or equal to 1
            medidicines_directions = [medicine for medicine in stripped_lines if len(medicine) > 1]

            if len(medidicines_directions)<2:
                return medidicines_directions[0]

            return medidicines_directions

        pattern_object = pattern_dict.get(field_name)
        if pattern_object:
            matches = re.findall(pattern_object['pattern'], self.text, flags=pattern_object['flags'])
            if len(matches) > 0:
                if field_name=='medicines' or field_name=='directions':
                    return medicines_directions_extractor(matches[0].strip())
